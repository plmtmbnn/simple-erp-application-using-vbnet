...
Public Class FormPurchasePayment
    ...
    Private Sub Load_Table()
        ...
        Try
            MySqlConn.Open()
            Dim Query As String
            Query = "select * from sdi_db.sdi_tb_purchase_payment"
    ...
    Public Shared Function get_product(id As Integer) As ArrayList
        ...
        Try
            ...
            While READER.Read
                ...
                product_price = READER.GetString("purchase_unit_price")
    ...
    Private Sub count_sales_inv(id As Integer)
        ...
        Try
            ...
            Dim Query As String
            Dim vendorId As Integer = 1
            Query = "select * from sdi_db.sdi_tb_purchase_invoice where vendor_id ='" & id & "' and status='0'"
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
            While READER.Read
                Dim vendor = READER.GetString("purchase_order_id")
                If (vendor = ComboBoxSO.Items.Count) Then
                Else
    ...
    Private Sub update_so_status(id As Integer)
        ...
        Try
            ...
            Dim Query As String
            Dim vendorId As Integer = 1
            Query = "update sdi_db.sdi_tb_purchase_invoice set status='1' where purchase_order_id =" & id
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
            MySqlConn.Close()
        Catch ex As Exception
    ...
    Private Sub load_id()
        ...
        Try
            ...
            READER = COMMAND.ExecuteReader
            While READER.Read
                Dim vendor = READER.GetString("purchase_payment_id")
                vendor += 1
                TextBoxIDSO.Text = vendor
            End While
            MySqlConn.Close()
        Catch ex As Exception
            MessageBox.Show("Gagal5!")
        Finally
            MySqlConn.Dispose()
        End Try
    End Sub
    Private Sub FormPurchasePayment_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        load_combo()
        load_id()
        Load_Table()
    ...
    Private Sub ButtonTambahkan_Click(sender As Object, e As EventArgs) Handles ButtonTambahkan.Click
        ...
        Try
            ...
            While READER.Read
                ...
                Dim kuantitas = 1
                Dim unitprice = READER.GetString("purchase_unit_price")
    ...
    Private Sub ComboBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox1.SelectedIndexChanged
        ...
        Try
            ...
            Dim Query As String
            Dim vendorId As Integer = 1
            Query = "select * from sdi_db.sdi_tb_vendore where vendor_name = '" & ComboBox1.Text & "'"
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
            While READER.Read
                Dim id = READER.GetString("vendor_id")
                TextBox1.Text = id
            End While
            MySqlConn.Close()
        Catch ex As Exception
            MessageBox.Show("Gagal7!")
        Finally
            MySqlConn.Dispose()
        End Try
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Try
            MySqlConn.Open()
            Dim Query As String
            Query = "insert into sdi_db.sdi_tb_purchase_payment (purchase_payment_id,purchase_invoice_id,purchase_payment_date,total_payment,status) 
            values ('" & Convert.ToInt32(TextBoxIDSO.Text) & "','" & Convert.ToInt32(ComboBoxSO.Text) & "','" & DateTimePicker1.Value.Date.ToString("yyyyMMdd") & "','" & Convert.ToInt32(TextBoxTotal.Text) & "','" & "0" & "')"
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
            ...
            MySqlConn.Close()
        Catch ex As Exception
            MessageBox.Show("Gagal8! " & ex.ToString)
        Finally
            MySqlConn.Dispose()
        End Try
        generate_pdf()
    ...
    Public Sub updateStok()
        ...
        Try
            ...
            For Each row As DataGridViewRow In DataGridView1.Rows
                MySqlConn.Open()
                If (Convert.ToInt32(row.Cells(2).Value) = 0) Then
                    Exit For
                Else
                    stok = get_productStok(Convert.ToInt32(row.Cells(0).Value)) + Convert.ToInt32(row.Cells(2).Value)
                    Query = "update sdi_db.sdi_tb_product set  
            stock='" & stok & "' where product_id='" & Convert.ToInt32(row.Cells(0).Value) & "'"
            ...
            Next
        Catch ex As Exception
            MessageBox.Show("Ga terupdate! " & ex.ToString)
        Finally
            MySqlConn.Dispose()
        End Try
    End Sub

    Public Shared Function get_productStok(id As Integer) As Integer
        Dim MySqlConn As MySqlConnection
        Dim COMMAND As MySqlCommand
        ...
        Try
            ...
            READER = COMMAND.ExecuteReader
            While READER.Read
                product = READER.GetString("stock")
    ...
    Public Shared Function get_Balance() As Double
        Dim MySqlConn As MySqlConnection
        Dim COMMAND As MySqlCommand
        ...
        Try
            ...
            READER = COMMAND.ExecuteReader
            While READER.Read
                product = Convert.ToDouble(READER.GetString("balance"))
            End While
            MySqlConn.Close()
        Catch ex As Exception
            MessageBox.Show("Gagal!")
    ...
    Public Sub createPP()
        ...
        Try
            MySqlConn.Open()
            Dim Query As String
            Dim total As Double = get_Balance() - Convert.ToInt32(TextBoxTotal.Text)
            Query = "insert into sdi_db.sdi_tb_cash (id,create_date,customer_receipt_id,purchase_payment_id,revenue,expenditure,balance) 
            values ('0','" & DateTimePicker1.Value.Date.ToString("yyyyMMdd") & "','" & DBNull.Value & "','" & Convert.ToInt32(TextBox1.Text) & "','" & 0 & "','" & Convert.ToDouble(TextBoxTotal.Text) & "','" & total & "')"
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
            Me.Hide()
            MySqlConn.Close()
    ...
    Public Sub generate_pdf()
        Try
            ...
            Dim writer As PdfWriter = PdfWriter.GetInstance(pdfDoc, New FileStream("D:\12S14026_POLMA\Semester 7\SISENT\Document\Pembelian\" + "PurchasePayment_" + TextBoxIDSO.Text + "_" + ComboBox1.Text + ".pdf", FileMode.Create))
            pdfDoc.Open()

            Dim font As New Font(FontFactory.GetFont("Helvetica", 16, Font.BOLD, BaseColor.BLACK))

            pdfDoc.Add(New Paragraph("-----------------------------Purchase Payment-----------------------------", font))

            Dim phrase1 As New Phrase("==========================================================================")
            pdfDoc.Add(phrase1)
            pdfDoc.Add(Chunk.NEWLINE)

            Dim phrase2 As New Phrase("PP Number    : " + TextBoxIDSO.Text)
            pdfDoc.Add(phrase2)
            pdfDoc.Add(Chunk.NEWLINE)

            Dim phrase3 As New Phrase("PP date      : " + DateTimePicker1.Value.Date.ToString("yyyy/MM/dd"))
            pdfDoc.Add(phrase3)
            pdfDoc.Add(Chunk.NEWLINE)

            Dim phrase4 As New Phrase("Disetok oleh : " + ComboBox1.Text)
            pdfDoc.Add(phrase4)
            pdfDoc.Add(Chunk.NEWLINE)
            pdfDoc.Add(Chunk.NEWLINE)
    ...
    Private Sub ButtonHapus_Click(sender As Object, e As EventArgs) 
        ...
        Try
            MySqlConn.Open()
            Dim Query As String
            ' Query = "delete from sdi_db.sdi_tb_purchase_invoice where id='" & Convert.ToUInt32(LabelID.Text) & "'"
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
    ...
    Private Sub ComboBoxSO_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBoxSO.SelectedIndexChanged
        ...
        Try
            ...
            Query = "select * from sdi_db.sdi_tb_purchase_invoice where purchase_order_id = " & Convert.ToInt32(ComboBoxSO.Text)
            COMMAND = New MySqlCommand(Query, MySqlConn)
            READER = COMMAND.ExecuteReader
            While READER.Read
