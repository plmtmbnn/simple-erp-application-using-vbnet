...
Public Class FormPurchasesOrder
    ...
    Private Sub Load_Table()
        ...
        Try
            MySqlConn.Open()
            Dim Query As String
            Query = "select * from sdi_db.sdi_tb_purchase_order"
    ...
    Private Sub FormPurchasesOrder_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        load_combo()
        Load_Table()
        load_id()
        ...
        Try
            ...
            For Each row As DataGridViewRow In DataGridView1.Rows
                ...
                Else
                    ...
                    Query = "insert into sdi_db.sdi_tb_purchase_order (id,purchase_order_id,vendor_id,product_id,purchase_order_date,quantity,total_payment,status) 
            values ('0','" & Convert.ToInt32(TextBoxIDSO.Text) & "','" & Convert.ToInt32(TextBox1.Text) & "','" & Convert.ToInt32(product_idz) & "','" & DateTimePicker1.Value.Date.ToString("yyyyMMdd") & "','" & Convert.ToInt32(kuantitasz) & "','" & Convert.ToInt32(totalz) & "','" & "0" & "')"
                    COMMAND = New MySqlCommand(Query, MySqlConn)
                    READER = COMMAND.ExecuteReader
                    MySqlConn.Close()
                End If
    ...
    Public Sub generate_pdf()
        Try
            ...
            Dim writer As PdfWriter = PdfWriter.GetInstance(pdfDoc, New FileStream("D:\12S14026_POLMA\Semester 7\SISENT\Document\Pembelian\" + "PurchaseOrder_" + TextBoxIDSO.Text + "_" + ComboBox1.Text + ".pdf", FileMode.Create))
            pdfDoc.Open()

            Dim font As New Font(FontFactory.GetFont("Helvetica", 16, font.Bold, BaseColor.BLACK))

            pdfDoc.Add(New Paragraph("-------------------------------Purchase Order----------------------------", font))

            Dim phrase1 As New Phrase("==========================================================================")
            pdfDoc.Add(phrase1)
            pdfDoc.Add(Chunk.NEWLINE)

            Dim phrase2 As New Phrase("PO Number    : " + TextBoxIDSO.Text)
            pdfDoc.Add(phrase2)
            pdfDoc.Add(Chunk.NEWLINE)

            Dim phrase3 As New Phrase("PO date      : " + DateTimePicker1.Value.Date.ToString("yyyy/MM/dd"))
            pdfDoc.Add(phrase3)
            pdfDoc.Add(Chunk.NEWLINE)
